...
async function sendOtpViaGateway(phone: string, code: string, requestId: string, username?: string): Promise<boolean> {
...
async function sendOtpFallback(code: string, phone: string, username?: string): Promise<boolean> {
  if (!env.TELEGRAM_BOT_TOKEN || env.TELEGRAM_BOT_TOKEN === 'replace-me') return false;
...
export async function registerAuthRoutes(app: FastifyInstance) {
  app.post('/request-otp', async (request, reply) => {
    ...
    const viaGateway: boolean = (await sendOtpViaGateway(body.phone, code, requestId, body.username)) === true;
    const viaBot: boolean = !viaGateway && ((await sendOtpFallback(code, body.phone, body.username)) === true);
  ...
  app.post('/verify', async (request, reply) => {
    ...
    if (!entry) return reply.code(400).send({ error: 'otp_not_requested' });
    if (body.requestId && entry.request_id !== body.requestId) return reply.code(400).send({ error: 'request_mismatch' });
    if (new Date() > new Date(entry.expires_at)) return reply.code(400).send({ error: 'otp_expired' });
    if (entry.attempts >= 5) return reply.code(429).send({ error: 'too_many_attempts' });
    ...
    if (entry.provider === 'mtproto' && entry.phone_code_hash && entry.mt_session) {
      try {
        ...
        if (e.message === 'two_factor_enabled') return reply.code(403).send({ error: '2fa_enabled' });
        if (e.message === 'invalid_code') return reply.code(401).send({ error: 'invalid_code' });
        return reply.code(401).send({ error: 'invalid_code' });
      }
    } else {
      const matches = await bcrypt.compare(body.code, entry.code_hash || '');
      await incOtpAttempts(body.phone);
      if (!matches) return reply.code(401).send({ error: 'invalid_code' });
    }
